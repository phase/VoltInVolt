...
void getValueAnyForm(State state, ir.Exp exp, Value result)
{
	...
	case VaArgExp:
		...
		break;
	default:
		throw panicUnhandled(exp, toString(exp.nodeType));
	}
}
...
void handleCast(State state, Location loc, Type newType, Value result)
{
	...
	throw panicUnhandled(loc, toString(newType.irType.nodeType));
}
...
void handlePostId(State state, ir.Postfix postfix, Value result)
{
	...
	if (ut !is null) {
		...
		throw panic(postfix.child.location, format("%s is not struct, array or pointer", toString(cast(void*) result.type)));
	}
}
...
void handleSliceTwo(State state, ir.Postfix postfix, Value result)
{
	...
	if (pt !is null) {
		...
		throw panicUnhandled(postfix, toString(left.type.irType.nodeType));
	}
