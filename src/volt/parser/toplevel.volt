...
body
{
	...
	switch (ts.peek.type) {
		...
		case TokenType.Unittest:
			...
			break;
		case TokenType.This:
			tlb.nodes ~= [cast(ir.Node) parseConstructor(ts)];
			break;
		case TokenType.Tilde:  // XXX: Is this unambiguous?
			tlb.nodes ~= [cast(ir.Node) parseDestructor(ts)];
			break;
		case TokenType.Union:
			tlb.nodes ~= [cast(ir.Node) parseUnion(ts)];
			break;
		case TokenType.Struct:
			tlb.nodes ~= [cast(ir.Node) parseStruct(ts)];
			break;
		case TokenType.Class:
			tlb.nodes ~= [cast(ir.Node) parseClass(ts)];
			break;
		case TokenType.Interface:
			tlb.nodes ~= [cast(ir.Node) parseInterface(ts)];
			break;
		case TokenType.Enum:
		...
		case TokenType.Mixin:
			...
			if (next == TokenType.Function) {
				tlb.nodes ~= [cast(ir.Node) parseMixinFunction(ts)];
			} else if (next == TokenType.Template) {
				tlb.nodes ~= [cast(ir.Node) parseMixinTemplate(ts)];
			} else {
		...
		case TokenType.Debug:
			tlb.nodes ~= [cast(ir.Node) parseConditionTopLevel(ts, inModule)];
			break;
...
ir.Node parseImport(TokenStream ts, bool inModule)
{
	...
	if (matchIf(ts, TokenType.Colon)) {
		...
		do {
			...
			first = false;
			ir.Import.Alias _alias;
			_alias._0 = parseIdentifier(ts);
			if (matchIf(ts, TokenType.Assign)) {
				// import a : b, <c = d>
				_alias._1 = parseIdentifier(ts);
			}
			_import.aliases ~= _alias;
