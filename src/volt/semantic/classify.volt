 ...
 */
version (none) bool mutableIndirection(ir.Type t)
{
...
bool isRefVar(ir.Exp exp)
{
	...
	return isRef(asVar.type, out dummy);
}
...
version (none) bool acceptableForUserAttribute(LanguagePass lp, ir.Scope current, ir.Type type)
{
...
version (none) int structSize(Location location, LanguagePass lp, ir.Struct s)
{
	int sizeAccumulator;
...
version (none) int unionSize(Location location, LanguagePass lp, ir.Union u)
{
	int sizeAccumulator;
...
bool effectivelyConst(ir.Type type)
{
	...
	return t == ir.StorageType.Kind.Const || t == ir.StorageType.Kind.Immutable;
}
...
version (none) bool isInFunction(Context ctx)
{
...
version (none) bool isImplicitlyConvertable(ir.Type from, ir.Type to)
{
 ...
 */
version (none) bool typesEqual(ir.Type a, ir.Type b)
{
	if (a is b) {
		return true;
	}
...
version (none) int typeToRuntimeConstant(LanguagePass lp, ir.Scope current, ir.Type type)
{
...
version (none) bool isPointerToClass(ir.Type t)
{
...
version (none) ir.Aggregate opOverloadableOrNull(ir.Type t)
{
